...
namespace SCReverser.Tests
{
    ...
    0xC3, 0x61, 0x6C, 0x75, 0x66
            ...
            Instruction[] instructions = reverser.GetInstructions(rawData, 0, rawData.Length).ToArray();
...
0F1C RET             []";
            ...
            string[] lines = decom.Trim().Split('\n');

            Assert.IsTrue(instructions.Length == lines.Length);

            for (int x = 0; x < lines.Length; x++)
            {
                string[] sp = lines[x].Split(' ');
